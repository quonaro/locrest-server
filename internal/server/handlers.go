...
func (f *Frontend) handleScript(w http.ResponseWriter, r *http.Request, localPort, remotePort int, targetHost, mode, httpAuth string) {
	cfg := f.cfg.Load()
	role, perms := roleAndPermissions(r, cfg, f.db)
	...
	if !f.rateLimiter.allow(ip) {
		...
		script.WriteError(w, "Rate limit exceeded", http.StatusTooManyRequests)
		return
	}
	if cfg.Tunnel.MaxSessions > 0 && f.store.Len() >= cfg.Tunnel.MaxSessions {
		...
		script.WriteError(w, "Server busy", http.StatusServiceUnavailable)
		return
	}
	...
	if err != nil || len(binaries) == 0 {
		...
		script.WriteError(w, "Client binaries not available, run 'lrs binary update'", http.StatusServiceUnavailable)
		return
	}
	...
	if targetHost != "" && !f.isAllowedTunnelHost(targetHost) {
		...
		script.WriteError(w, "target host is not allowed", http.StatusBadRequest)
		return
	}
	...
	if requestedSubdomain != "" && f.isReservedSubdomain(requestedSubdomain) {
		...
		script.WriteError(w, "subdomain is reserved", http.StatusBadRequest)
		return
	}
	...
	if allowedIPsRaw != "" {
		...
		if err != nil {
			...
			script.WriteError(w, err.Error(), http.StatusBadRequest)
			return
		}
	}
	...
	if serverPort <= 0 {
		...
		script.WriteError(w, "Port already in use", http.StatusConflict)
		return
	}
	...
	if err != nil {
		...
		script.WriteError(w, msg, status)
		return
	}
	...
	if err != nil {
		...
		script.WriteError(w, "Script generation failed", http.StatusInternalServerError)
		return
	}
